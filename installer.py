...
def check_env_file():
    ...
    with open(".env.template", "r", encoding="utf-8") as template_file:
    ...
    template_sections = [
        line.strip() for line in template_lines if line.startswith("[")
    ]
    env_sections = [line.strip() for line in env_lines if line.startswith("[")]

    for section in template_sections:
        if section not in env_sections:
            print(f"Missing section header: {section}")
            print("Please copy the contents of .env.template to .env file")
            abort()

    for i, section in enumerate(env_sections):
        if section not in template_sections:
            print(f"Misplaced section header: {section}")
            misplaced = True
        elif section != template_sections[i]:
            print(
                f"Misplaced section header: {section}. "
                f"Should be after {template_sections[i-1]} and before {template_sections[i+1]}"
            )
            misplaced = True
    if misplaced:
        print("Please copy the contents of .env.template to .env file")
