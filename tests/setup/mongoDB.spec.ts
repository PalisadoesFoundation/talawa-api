...
describe("Setup -> mongoDB", () => {
  ...
  it("should prompt for and connect to new URL if existing is not confirmed or unavailable", async () => {
    vi.spyOn(module, "checkExistingMongoDB").mockImplementationOnce(() => Promise.resolve(null))
    const newUrl = "mongodb://testUrl-new";
    vi.spyOn(inquirer, "prompt").mockResolvedValueOnce({ keepValues: false });
    vi.spyOn(module, "askForMongoDBUrl").mockImplementationOnce(() => Promise.resolve(newUrl))
    vi.spyOn(module, "checkConnection").mockImplementationOnce(() => Promise.resolve(true))
