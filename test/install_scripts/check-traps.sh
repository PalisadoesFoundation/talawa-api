#!/bin/bash
##############################################################################
# check-traps.sh - Verify every *.test.sh in test/install_scripts has an EXIT trap
#
# Used by lefthook pre-commit to ensure temp resources are cleaned on exit.
# Usage: ./check-traps.sh   (from repo root or test/install_scripts)
...
if [ "$FAILED" -eq 1 ]; then
    ...
    echo "See test/install_scripts/README.md (Environment isolation, Execution model)."
    exit 1
fi
exit 0
