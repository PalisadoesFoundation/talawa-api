...
describe("Setup", () => {
	...
	it("should restore .env on SIGINT (Ctrl+C) and exit with code 0 when backup exists", async () => {
		...
		// Manually set backupCreated to true since we might be interrupting before envFileBackup runs
		SetupModule.__test__setBackupCreated(true);

		// Emit SIGINT to trigger the handler
		const listeners = process.listeners("SIGINT");
		const handler = listeners[listeners.length - 1];
		if (!handler) throw new Error("No SIGINT listener found");
		// @ts-expect-error
		...
		expect(fsRenameSpy).toHaveBeenCalledWith(".env.tmp", ".env");
	...
	it("should exit with code 1 when restoreLatestBackup fails", async () => {
		...
		// Import test helper to set backupCreated state
		const { __test__setBackupCreated } = await import("scripts/setup/setup");
