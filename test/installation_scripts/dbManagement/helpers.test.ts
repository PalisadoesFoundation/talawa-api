...
beforeAll(async () => {
	...
	await import("test/envConfigSchema");
	// testEnvConfig is unused but assigned for potential future use or side effects

	vi.resetModules();
	// Re-apply mock after resetModules just in case
	vi.doMock("env-schema", async () => {
		return {
			default: vi.fn(() => {
				return {
		...
		db.transaction = (async (
			...
			const tx = {
				execute: vi.fn().mockResolvedValue([
					{ tablename: "table1" },
					{ tablename: "users" }, // should skip users truncation
				]),
			};
			await cb(tx);
