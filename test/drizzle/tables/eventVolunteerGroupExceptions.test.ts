...
describe("eventVolunteerGroupExceptionsTable", () => {
	describe("eventVolunteerGroupExceptionsTableInsertSchema", () => {
		...
		it("should accept null for optional createdBy and updatedBy", () => {
			const result = eventVolunteerGroupExceptionsTableInsertSchema.safeParse({
				...validData,
				createdBy: null,
				updatedBy: null,
		...
		describe("eventVolunteerGroupExceptionsTable Relations", () => {
			beforeAll(() => {
				...
				(
					...
					one: (table: Table, config?: CapturedRelation["config"]) => {
						...
						if (
							name === "event_volunteer_group_exceptions.created_by:users.id"
						) {
							capturedRelations.createdByUser = { table, config, type: "one" };
						}
						if (
							name === "event_volunteer_group_exceptions.updated_by:users.id"
						) {
			...
			describe("volunteerGroup relations", () => {
				...
				it("should have correct fields and references", () => {
					...
					const references =
						capturedRelations?.volunteerGroup?.config.references;
			...
			describe("recurringEventInstance relations", () => {
				...
				it("should have correct fields and references", () => {
					const fields =
						capturedRelations?.recurringEventInstance?.config.fields;
					const references =
			...
			describe("createdByUser relations", () => {
				...
				it("should have correct fields and references", () => {
					...
					const references =
						capturedRelations?.createdByUser?.config.references;
					expect(fields).toEqual([
						eventVolunteerGroupExceptionsTable.createdBy,
					]);
			...
			describe("updatedByUser relations", () => {
				...
				it("should have correct fields and references", () => {
					...
					const references =
						capturedRelations?.updatedByUser?.config.references;
					expect(fields).toEqual([
						eventVolunteerGroupExceptionsTable.updatedBy,
					]);
					expect(references).toEqual([usersTable.id]);
				});
			});
