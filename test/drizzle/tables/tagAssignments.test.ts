...
import { organizationsTable } from '../../../src/drizzle/tables/organizations'; // <--- NEW IMPORT
  ...
  describe('Database Operations (Integration)', () => {
    it('should successfully insert and retrieve a record', async () => {
      ...
      const [organization] = await db.insert(organizationsTable).values({
        ...
        slug: faker.lorem.slug() + "-" + faker.string.uuid(), // Ensure unique slug
        isPublic: true
      } as any).returning();
      ...
      const [tag] = await db.insert(tagsTable).values({
        ...
        organizationId: organization.id // <--- Use the real ID here
      } as any).returning();
      ...
      const [result] = await db.insert(tagAssignmentsTable)
      ...
      await db.delete(tagAssignmentsTable)
        .where(sql`${tagAssignmentsTable.assigneeId} = ${assignee.id} AND ${tagAssignmentsTable.tagId} = ${tag.id}`);
    });
  });
});