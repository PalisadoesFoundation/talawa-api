...
describe("src/drizzle/tables/actionItems.ts - Table Definition Tests", () => {
	...
	describe("Index Configuration", () => {
		it("should have appropriate indexes defined", () => {
			...
			expect(tableConfig.indexes.length).toBeGreaterThanOrEqual(8);
		});

		it("should have an index on assignedAt column", () => {
			const tableConfig = getTableConfig(actionItemsTable);
			const assignedAtIndex = tableConfig.indexes.find((idx) =>
				idx.config.columns.some(
					(col) => "name" in col && col.name === "assigned_at",
				),
			);
			expect(assignedAtIndex).toBeDefined();
			expect(assignedAtIndex?.config.unique).toBeFalsy();
		});

		it("should have an index on volunteerId column", () => {
			const tableConfig = getTableConfig(actionItemsTable);
			const volunteerIdIndex = tableConfig.indexes.find((idx) =>
				idx.config.columns.some(
					(col) => "name" in col && col.name === "volunteer_id",
				),
			);
			expect(volunteerIdIndex).toBeDefined();
			expect(volunteerIdIndex?.config.unique).toBeFalsy();
		});

		it("should have an index on volunteerGroupId column", () => {
			const tableConfig = getTableConfig(actionItemsTable);
			const volunteerGroupIdIndex = tableConfig.indexes.find((idx) =>
				idx.config.columns.some(
					(col) => "name" in col && col.name === "volunteer_group_id",
				),
			);
			expect(volunteerGroupIdIndex).toBeDefined();
			expect(volunteerGroupIdIndex?.config.unique).toBeFalsy();
		});

		it("should have an index on categoryId column", () => {
			const tableConfig = getTableConfig(actionItemsTable);
			const categoryIdIndex = tableConfig.indexes.find((idx) =>
				idx.config.columns.some(
					(col) => "name" in col && col.name === "category_id",
				),
			);
			expect(categoryIdIndex).toBeDefined();
			expect(categoryIdIndex?.config.unique).toBeFalsy();
		});

		it("should have an index on completionAt column", () => {
			const tableConfig = getTableConfig(actionItemsTable);
			const completionAtIndex = tableConfig.indexes.find((idx) =>
				idx.config.columns.some(
					(col) => "name" in col && col.name === "completion_at",
				),
			);
			expect(completionAtIndex).toBeDefined();
			expect(completionAtIndex?.config.unique).toBeFalsy();
		});
		...
		it("should have an index on createdAt column", () => {
			...
			expect(createdAtIndex?.config.unique).toBeFalsy();
		});
		...
		it("should have an index on organizationId column", () => {
			const tableConfig = getTableConfig(actionItemsTable);
			const organizationIdIndex = tableConfig.indexes.find((idx) =>
				idx.config.columns.some(
					(col) => "name" in col && col.name === "organization_id",
				),
			);
			expect(organizationIdIndex).toBeDefined();
			expect(organizationIdIndex?.config.unique).toBeFalsy();
		});
	});
