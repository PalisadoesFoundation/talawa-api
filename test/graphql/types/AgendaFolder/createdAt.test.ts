...
import type { introspection } from "../gql.tada"; const gql = initGraphQLTada<{
...
async function cleanup(
    authToken: string,
    {
        orgId,
        eventId,
    }: { orgId: string; eventId: string; folderId?: string },
) {
    try {
    ...
    describe("Authentication and Authorization", () => {
        it("should throw unauthenticated error when client is not authenticated", async () => {
            ...
            try {
                ...
                const result = await mercuriusClient.query(Query_agendaFolder_createdAt, {
                    variables: {
                        id: folderId,
                    },
                });
        ...
        it("should throw unauthorized_action error when user is not organization admin", async () => {
            ...
            try {
                ...
                expect(result.errors).toBeDefined();
                expect(result.errors?.[0]?.extensions?.code).toBe("unauthorized_action_on_arguments_associated_resources");
            } finally {
                await cleanup(adminAuth.token, { orgId, eventId, folderId });
            }
        });
