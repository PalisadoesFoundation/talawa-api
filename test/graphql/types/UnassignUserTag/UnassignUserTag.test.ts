...
import "~/src/graphql/types/UnassignUserTag/UnassignUserTag";

suite("UnassignUserTag Type", () => {
	describe("Schema Registration", () => {
		test("should be registered in the GraphQL schema", () => {
			const type = schema.getType("UnassignUserTag") as GraphQLObjectType;
			expect(type).toBeDefined();
			expect(type.name).toBe("UnassignUserTag");
		});

		test("should have the correct type description", () => {
			const type = schema.getType("UnassignUserTag") as GraphQLObjectType;
			expect(type.description).toBe("Represents a tag unassigned from a user.");
		});
