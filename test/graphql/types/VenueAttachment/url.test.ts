...
import "~/src/graphql/types/VenueAttachment/url";
import { createMockGraphQLContext } from "test/_Mocks_/mockContextCreator/mockContextCreator";

// Get the url resolver from the schema
const venueAttachmentType = schema.getType("VenueAttachment");
if (!venueAttachmentType || !("getFields" in venueAttachmentType)) {
	throw new Error("VenueAttachment type not found or is not an object type");
}
const urlField = (venueAttachmentType as GraphQLObjectType).getFields().url;
...
const urlResolver = urlField.resolve as (
	parent: VenueAttachment,
	args: Record<string, never>,
	ctx: GraphQLContext,
) => Promise<string>;

describe("VenueAttachment.url field resolver - Unit tests", () => {
	let ctx: GraphQLContext;
	let mockAttachment: VenueAttachment;
