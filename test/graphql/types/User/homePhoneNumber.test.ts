...
suite("User field homePhoneNumber", () => {
		...
		try {
			if (createdUserIds.length > 0) {
				...
				assertToBeNonNullish(
					adminSignIn.data.signIn?.authenticationToken,
				);
				const token =
					adminSignIn.data.signIn.authenticationToken;
	...
	test('returns "unauthenticated" when client is not authenticated', async () => {
		...
		const createUser = await mercuriusClient.mutate(
			Mutation_createUser,
			{
		...
		expect(result.errors).toEqual(
			expect.arrayContaining<TalawaGraphQLFormattedError>([
				expect.objectContaining({
					extensions:
						expect.objectContaining<UnauthenticatedExtensions>({
	...
	test("returns homePhoneNumber for self access", async () => {
		...
		const userRes = await mercuriusClient.mutate(
			Mutation_createUser,
			{
				headers: { authorization: `bearer ${token}` },
				variables: {
		...
		assertToBeNonNullish(
			userRes.data.createUser?.authenticationToken,
		);
		...
		expect(result.data.user?.homePhoneNumber).toBe(
			"111-222-3333",
		);
	});
