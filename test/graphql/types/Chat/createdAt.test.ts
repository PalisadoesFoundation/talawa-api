...
suite("Chat field createdAt", () => {
	...
	test('results in a graphql error with "unauthenticated" extensions code when auth token belongs to a deleted user', async () => {
		// Create a temporary user and then delete the user record to simulate token for a missing user
		const tempUser = await createTestUser(adminAuthToken, "regular");
		const tempUserId = tempUser.userId;
		const tempUserAuthToken = tempUser.authToken;

		// Delete the user directly using admin to remove DB record but not the token
		await mercuriusClient.mutate(Mutation_deleteUser, {
			headers: { authorization: `bearer ${adminAuthToken}` },
			variables: { input: { id: tempUserId } },
		});

		// Now query the chat with the deleted user's token - should hit currentUser === undefined branch
		const result = await mercuriusClient.query(Query_chat_with_createdAt, {
			headers: { authorization: `bearer ${tempUserAuthToken}` },
			variables: {
				input: {
					id: testChatId,
				},
