...
import { resolveUpdatedAt } from "~/src/graphql/types/VolunteerGroup/updatedAt";
...
describe("VolunteerGroup.updatedAt resolver", () => {
	...
	test("throws an unauthenticated error if user is not authenticated or not found", async () => {
		...
		await expect(
			resolveUpdatedAt(mockVolunteerGroup, {}, unauthenticatedCtx),
		).rejects.toThrow(TalawaGraphQLError);

		mocks.drizzleClient.query.usersTable.findFirst.mockResolvedValue(undefined);

		await expect(resolveUpdatedAt(mockVolunteerGroup, {}, ctx)).rejects.toThrow(
			TalawaGraphQLError,
		);
	});
	...
	test("returns updatedAt Date", async () => {
		mocks.drizzleClient.query.eventsTable.findFirst.mockResolvedValue({
			id: "event123",
		});
		...
		const result = await resolveUpdatedAt(mockVolunteerGroup, {}, ctx);
