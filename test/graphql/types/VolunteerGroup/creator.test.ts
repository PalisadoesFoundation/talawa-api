...
import { resolveCreator } from "~/src/graphql/types/VolunteerGroup/creator";
...
describe("VolunteerGroup.creator resolver", () => {
	...
	test("throws an unauthenticated error if user is not authenticated or not found", async () => {
		...
		await expect(
			resolveCreator(mockVolunteerGroup, {}, unauthenticatedCtx),
		).rejects.toThrow(TalawaGraphQLError);

		mocks.drizzleClient.query.usersTable.findFirst.mockResolvedValue(undefined);

		await expect(resolveCreator(mockVolunteerGroup, {}, ctx)).rejects.toThrow(
			TalawaGraphQLError,
		);
	});
	...
	test("returns null if creatorId is null", async () => {
		mocks.drizzleClient.query.eventsTable.findFirst.mockResolvedValue({
			id: "event123",
		});
		mocks.drizzleClient.query.usersTable.findFirst.mockResolvedValue({
		...
		const result = await resolveCreator(
			{ ...mockVolunteerGroup, creatorId: null },
			{},
			ctx,
		);
	...
	test("returns creator", async () => {
		...
		const result = await resolveCreator(mockVolunteerGroup, {}, ctx);
	...
	test("returns if current user is creator", async () => {
		mocks.drizzleClient.query.eventsTable.findFirst.mockResolvedValue({
			id: "event123",
		});
		mocks.drizzleClient.query.usersTable.findFirst.mockResolvedValue({
		...
		const result = await resolveCreator(
			{ ...mockVolunteerGroup, creatorId: "user456" },
			{},
			ctx,
		);
		expect(result).toEqual({
	...
	test("throws an error if creator not found", async () => {
		mocks.drizzleClient.query.eventsTable.findFirst.mockResolvedValue({
			id: "event123",
