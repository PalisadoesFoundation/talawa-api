...
suite("Mutation field createEventVolunteerGroupAssignments", () => {
	suite("when the client is not authenticated", () => {
		test("should return an error with unauthenticated code", async () => {
			...
			expect(result.data?.createEventVolunteerGroupAssignments).toBeNull();
			expect(result.errors).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						extensions: expect.objectContaining({ code: "unauthenticated" }),
						path: ["createEventVolunteerGroupAssignments"],
					}),
				]),
			);
		});
	...
	suite(
		...
		() => {
			test("should return the created volunteer group assignment", async () => {
				...
				expect(
					result.data?.createEventVolunteerGroupAssignments,
				).not.toBeNull();
				expect(result.data?.createEventVolunteerGroupAssignments).toEqual(
					expect.objectContaining({
						group: { id: groupId },
