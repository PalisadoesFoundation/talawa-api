...
import {
	...
	Mutation_createUser,
	Mutation_deleteUser,
	Mutation_updateEventVolunteerGroupAssignments,
	Query_signIn,
} from "../documentNodes";
...
suite("Mutation field updateEventVolunteerGroupAssignments", () => {
	suite("when the client is not authenticated", () => {
		test("should return an error with unauthenticated code", async () => {
			...
			expect(result.data?.updateEventVolunteerGroupAssignments).toBeNull();
			expect(result.errors).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						extensions: expect.objectContaining({ code: "unauthenticated" }),
						path: ["updateEventVolunteerGroupAssignments"],
					}),
				]),
			);
		});
	...
	suite(
		...
		() => {
			test("should return the created volunteer group assignment", async () => {
				...
				expect(
					result.data?.updateEventVolunteerGroupAssignments,
				).not.toBeNull();
				expect(result.data?.updateEventVolunteerGroupAssignments).toEqual(
					expect.objectContaining({
						group: { id: groupId },
