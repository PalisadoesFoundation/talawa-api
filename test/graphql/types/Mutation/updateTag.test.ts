...
suite("Mutation field updateTag", () => {
  ...
  suite("when the folderId tag does not belong to the same organization", () => {
  ...
  suite("when updating tag name to one that already exists", () => {
    test("should return an error with forbidden_action_on_arguments_associated_resources extensions code", async () => {
      ...
      const createTag1Result = await mercuriusClient.mutate(Mutation_createTag, {
        headers: { authorization: `bearer ${authToken}` },
        variables: {
          input: {
      ...
      const createTag2Result = await mercuriusClient.mutate(Mutation_createTag, {
        headers: { authorization: `bearer ${authToken}` },
        variables: {
          input: {
            name: "Other Tag Name",
  ...
  suite("when the client is authorized and the tag is updated successfully", () => {
