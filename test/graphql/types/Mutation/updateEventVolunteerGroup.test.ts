...
suite("Mutation field updateEventVolunteerGroup", () => {
	suite("when the client is not authenticated", () => {
		test("should return an error with unauthenticated code", async () => {
			const result = await mercuriusClient.mutate(
				Mutation_updateEventVolunteerGroup,
				{
					variables: {
						input: {
							id: groupId,
							name: `Updated Group ${faker.string.uuid()}`,
			...
			expect(result.data.updateEventVolunteerGroup).toBeNull();
		...
		test("should return an error with arguments_associated_resources_not_found", async () => {
			const result = await mercuriusClient.mutate(
				Mutation_updateEventVolunteerGroup,
				{
					...
					variables: {
						input: {
							id: faker.string.uuid(), // non-existent group
							name: `Updated Group ${faker.string.uuid()}`,
							leaderId: adminUserId,
							maxVolunteerCount: 15,
						},
			...
			const result = await mercuriusClient.mutate(
				Mutation_updateEventVolunteerGroup,
				{
					...
					variables: {
						input: {
							id: groupId,
							name: `Unauthorized Update ${faker.string.uuid()}`,
							leaderId: regularUserId,
							maxVolunteerCount: 20,
						},
					},
				},
			);
				...
				Mutation_updateEventVolunteerGroup,
				{
					...
					variables: {
						input: {
							...
							name: `Updated Group ${faker.string.uuid()}`,
							leaderId: adminUserId,
							maxVolunteerCount: -5, // Invalid value
						},
					},
				},
			);
	...
	suite("when volunteer group is updated successfully by org admin", () => {
		test("should return the updated volunteer group", async () => {
			...
			const newGroupName = `Admin Updated Group ${faker.string.uuid()}`;
			const newMaxVolunteers = 25;
				...
				Mutation_updateEventVolunteerGroup,
				{
					...
					variables: {
						input: {
							...
							name: newGroupName,
							leaderId: adminUserId,
							maxVolunteerCount: newMaxVolunteers,
						},
					},
				},
			);

			expect(result.data?.updateEventVolunteerGroup).not.toBeNull();
			expect(result.data?.updateEventVolunteerGroup).toEqual(
				expect.objectContaining({
					id: groupId,
					name: newGroupName,
					maxVolunteerCount: newMaxVolunteers,
