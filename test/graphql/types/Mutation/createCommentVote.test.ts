...
async function createTestComment(authToken: string): Promise<{
		...
		Mutation_createOrganization,
		{
			...
			variables: {
				input: {
					...
					description: "Test organization for comment vote tests",
					countryCode: "us",
					state: "CA",
					city: "San Francisco",
					postalCode: "94101",
	...
	const createPostResult = await mercuriusClient.mutate(
		(await import("../documentNodes")).Mutation_createPost,
		{
			headers: { authorization: `bearer ${authToken}` },
			variables: {
				input: {
					...
					attachments: [
						{
							mimetype: "IMAGE_PNG",
							objectName: `test-object-${faker.string.ulid()}`,
							name: "test-image.png",
							fileHash: `test-hash-${faker.string.ulid()}`,
						},
					],
				},
			},
				...
				.where(
					(fields: any, operators: any) =>
						operators.and(
							...
							operators.eq(fields.memberId, regularUserId),
						),
				);
