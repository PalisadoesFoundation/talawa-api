...
suite("Mutation field updateActionForInstance", () => {
  ...
  suite("when the database insert operation fails", () => {
    test("should return an error with unexpected code", async () => {
      ...
      // Get an instance ID
      const instances = await server.drizzleClient.query.recurringEventInstancesTable.findMany({
      ...
      // Create action item
      const actionItemResult = await mercuriusClient.mutate(Mutation_createActionItem, {
        headers: { authorization: `bearer ${authToken}` },
        variables: {
          input: {
            categoryId: categoryId,
  ...
  // 6. Successful update.
  suite("when action item is updated successfully for an instance", () => {
