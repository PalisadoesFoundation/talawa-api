...
import { resolveCreatedAt } from "~/src/graphql/types/VolunteerGroupAssignment/createdAt";
...
describe("VolunteerGroupAssignment.createdAt resolver", () => {
	...
	test("throws an unauthenticated error if user is not authenticated", async () => {
		...
		await expect(
			resolveCreatedAt(mockVolunteerGroupAssignment, {}, unauthenticatedCtx),
		).rejects.toThrow(
	...
	test("throws an error if volunteer group does not exist", async () => {
		...
		await expect(
			resolveCreatedAt(mockVolunteerGroupAssignment, {}, ctx),
		).rejects.toThrow(
			expect.objectContaining({
				extensions: expect.objectContaining({
	...
	test("returns createdAt when user is administrator", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns createdAt when user is organization member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns createdAt when user is organization admin member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("handles database query chain correctly", async () => {
		...
		await resolveCreatedAt(mockVolunteerGroupAssignment, {}, ctx);
