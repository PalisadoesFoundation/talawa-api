...
import { resolveUpdator } from "~/src/graphql/types/VolunteerGroupAssignment/updator";
...
describe("VolunteerGroupAssignment.updater resolver", () => {
	...
	test("throws an unauthenticated error if user is not authenticated", async () => {
		...
		await expect(
			resolveUpdator(mockVolunteerGroupAssignment, {}, unauthenticatedCtx),
		).rejects.toThrow(
	...
	test("throws an error if volunteer group does not exist", async () => {
		...
		await expect(
			resolveUpdator(mockVolunteerGroupAssignment, {}, ctx),
		).rejects.toThrow(
			expect.objectContaining({
				extensions: expect.objectContaining({
	...
	test("returns null when updaterId is null", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
		...
		const result = await resolveUpdator(
	...
	test("throws an unexpected error when updater user does not exist but updaterId is not null", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
		...
		expect(ctx.log.error).toHaveBeenCalledWith(
			"Postgres select operation returned an empty array for an assignment updater id that isn't null.",
		);
	});

	test("returns updater user when user is administrator", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
		...
		const result = await resolveUpdator(mockVolunteerGroupAssignment, {}, ctx);
	...
	test("returns updater user when user is organization member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns updater user when user is organization admin member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("handles database query chain correctly", async () => {
		...
		await resolveUpdator(mockVolunteerGroupAssignment, {}, ctx);
