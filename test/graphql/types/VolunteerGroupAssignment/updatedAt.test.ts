...
import { resolveUpdatedAt } from "~/src/graphql/types/VolunteerGroupAssignment/updatedAt";
...
describe("VolunteerGroupAssignment.updatedAt resolver", () => {
	...
	test("throws an unauthenticated error if user is not authenticated", async () => {
		...
		await expect(
			resolveUpdatedAt(mockVolunteerGroupAssignment, {}, unauthenticatedCtx),
		).rejects.toThrow(
	...
	test("throws an error if volunteer group does not exist", async () => {
		...
		await expect(
			resolveUpdatedAt(mockVolunteerGroupAssignment, {}, ctx),
		).rejects.toThrow(
			expect.objectContaining({
				extensions: expect.objectContaining({
	...
	test("returns updatedAt when user is administrator", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns updatedAt when user is organization member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns updatedAt when user is organization admin member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("handles database query chain correctly", async () => {
		...
		await resolveUpdatedAt(mockVolunteerGroupAssignment, {}, ctx);
