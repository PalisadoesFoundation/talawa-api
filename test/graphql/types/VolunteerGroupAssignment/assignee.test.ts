...
import { resolveAssignee } from "~/src/graphql/types/VolunteerGroupAssignment/assignee";
...
describe("VolunteerGroupAssignment.assignee resolver", () => {
	...
	test("throws an unauthenticated error if user is not authenticated", async () => {
		...
		await expect(
			resolveAssignee(mockVolunteerGroupAssignment, {}, unauthenticatedCtx),
		).rejects.toThrow(
	...
	test("throws an error if volunteer group does not exist", async () => {
		...
		await expect(
			resolveAssignee(mockVolunteerGroupAssignment, {}, ctx),
		).rejects.toThrow(
			expect.objectContaining({
				extensions: expect.objectContaining({
	...
	test("throws an unexpected error when assignee user does not exist but assigneed is not null", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns assignee user when user is administrator", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
		...
		const result = await resolveAssignee(mockVolunteerGroupAssignment, {}, ctx);
	...
	test("returns assignee user when user is organization member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("returns assignee user when user is organization admin member", async () => {
		const mockResult = {
			volunteerGroup: {
				id: "group123",
				eventId: "event123",
	...
	test("handles database query chain correctly", async () => {
		...
		await resolveAssignee(mockVolunteerGroupAssignment, {}, ctx);
