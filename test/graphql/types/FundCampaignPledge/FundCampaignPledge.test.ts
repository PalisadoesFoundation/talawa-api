...
describe('resolveUpdater', () => {
  ...
  test('throws an unauthenticated error if user is not authenticated or not found', async () => {
    const unauthenticatedCtx = { ...mockCtx, currentClient: { isAuthenticated: false as const } };

    await expect(resolveUpdater(mockFundCampaignPledge, {}, unauthenticatedCtx)).rejects.toThrow(TalawaGraphQLError);

    mockDrizzleClient.query.usersTable.findFirst.mockResolvedValue(undefined);

    await expect(resolveUpdater(mockFundCampaignPledge, {}, mockCtx)).rejects.toThrow(TalawaGraphQLError);
  });

  test('throws an unexpected error if campaign does not exist', async () => {
    mockDrizzleClient.query.usersTable.findFirst.mockResolvedValue({ id: 'user123', role: 'administrator' });
    mockDrizzleClient.query.fundCampaignsTable.findFirst.mockResolvedValue(undefined);
  ...
  test('throws an unauthorized error if user is not an administrator', async () => {
    mockDrizzleClient.query.usersTable.findFirst.mockResolvedValue({ id: 'user123', role: 'member' });
    mockDrizzleClient.query.fundCampaignsTable.findFirst.mockResolvedValue({
      fund: { organization: { membershipsWhereOrganization: [{ role: 'member' }] } },
    });
  ...
  test('returns null if updaterId is null', async () => {
    mockDrizzleClient.query.usersTable.findFirst.mockResolvedValue({ id: 'user123', role: 'administrator' });
    mockDrizzleClient.query.fundCampaignsTable.findFirst.mockResolvedValue({
      fund: { organization: { membershipsWhereOrganization: [{ role: 'administrator' }] } },
    });

    const result = await resolveUpdater({ ...mockFundCampaignPledge, updaterId: null }, {}, mockCtx);
  ...
  test('returns current user if they are the updater', async () => {
    ...
    const result = await resolveUpdater({ ...mockFundCampaignPledge, updaterId: 'user123' }, {}, mockCtx);
  ...
  test('ensures usersTable query is called with correct user ID', async () => {
    mockDrizzleClient.query.usersTable.findFirst.mockImplementation(({ where }) => {
      return where(
        { id: 'user123' },
  ...
  test('ensures fundCampaignsTable membership check uses correct member ID', async () => {
    mockDrizzleClient.query.fundCampaignsTable.findFirst.mockImplementation(({ where }) => {
      return where(
        { id: 'campaign1' },
        { eq: (a:string, b:string) => a === b }
      ) && Promise.resolve({
        fund: {
          organization: {
            membershipsWhereOrganization: [{ role: 'administrator', memberId: 'user123' }],
          },
        },
      });
    });
    await resolveUpdater(mockFundCampaignPledge, {}, mockCtx);
    expect(mockDrizzleClient.query.fundCampaignsTable.findFirst).toHaveBeenCalled();
  });
  ...
  test('ensures where condition is applied correctly for membershipsWhereOrganization', async () => {
    ...
    const callArgs = mockDrizzleClient.query.fundCampaignsTable.findFirst.mock.calls[0]?.[0];
    const whereFn = callArgs?.with?.fund?.with?.organization?.with?.membershipsWhereOrganization?.where;
    ...
    if (whereFn) {
      ...
      expect(mockOperators.eq).toHaveBeenCalledWith(mockFields.memberId, 'user123');
    }
  });
});
