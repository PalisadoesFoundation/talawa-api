...
describe("FundCampaignPledge Resolver - pledger Field", () => {
  ...
  it("should throw unexpected error when fund campaign does not exist", async () => {
    mocks.drizzleClient.query.usersTable.findFirst.mockResolvedValue({ role: "member" });
    mocks.drizzleClient.query.fundCampaignsTable.findFirst.mockResolvedValue(undefined);
  ...
  it("should query membershipsWhereOrganization with currentUserId as memberId", async () => {
    ...
    const callArgs = calls[0]?.[0] as any;
    const whereFunction =
      callArgs?.with?.fund?.with?.organization?.with?.membershipsWhereOrganization?.where;
  ...
  it("should select role column as true in membershipsWhereOrganization", async () => {
    ...
    const membershipsConfig =
      callArgs?.with?.fund?.with?.organization?.with?.membershipsWhereOrganization;
  ...
  it("should query usersTable with pledgerId", async () => {
    ...
    expect(mocks.drizzleClient.query.usersTable.findFirst).toHaveBeenCalledTimes(2);

    const calls = (
      mocks.drizzleClient.query.usersTable.findFirst as ReturnType<typeof vi.fn>
    ).mock.calls;
    const callArgs = calls[1]?.[0] as any;
    const whereFunction = callArgs?.where;
