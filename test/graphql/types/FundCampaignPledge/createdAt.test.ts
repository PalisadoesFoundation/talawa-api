...
describe("FundCampaignPledge Resolver - createdAt Field", () => {
	...
	it("should return createdAt when user is an organization administrator", async () => {
		...
		await expect(
			createdAtResolver(mockFundCampaignPledge, {}, ctx as GraphQLContext),
		).resolves.toEqual(mockFundCampaignPledge.createdAt);
	});
	...
	it("should query fundCampaignsTable with campaignId", async () => {
		...
		await createdAtResolver(mockFundCampaignPledge, {}, ctx as GraphQLContext);
		...
		// Verify it was called with campaignId from mockFundCampaignPledge
