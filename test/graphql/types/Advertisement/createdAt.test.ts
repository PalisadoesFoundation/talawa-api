...
import "~/src/graphql/types/Advertisement/createdAt";
import { createMockGraphQLContext } from "test/_Mocks_/mockContextCreator/mockContextCreator";

// Get the createdAt resolver from the schema
const advertisementType = schema.getType("Advertisement");
if (!advertisementType || !("getFields" in advertisementType)) {
	throw new Error("Advertisement type not found or is not an object type");
}
const createdAtField = (advertisementType as GraphQLObjectType).getFields()
	.createdAt;
// Guard: ensure the resolver exists and is callable
if (typeof createdAtField?.resolve !== "function") {
	throw new Error("createdAt field resolver is not a function");
}
...
describe("Advertisement.createdAt field resolver - Unit tests", () => {
	...
	describe("Authentication checks", () => {
		it("should throw unauthenticated error when user is not authenticated", async () => {
			...
			await expect(
				createdAtResolver(mockAdvertisement, {}, ctx),
		...
		it("should throw unauthenticated error when currentUser is undefined (user not found in database)", async () => {
			...
			expect(
				eqMock.mock.calls.some(
					([, rhs]) => rhs === mockAdvertisement.organizationId,
				),
			).toBe(true);
		});
	});
	...
	describe("Successful resolution", () => {
		it("should return createdAt when user is system administrator", async () => {
			...
			const result = await createdAtResolver(mockAdvertisement, {}, ctx);

			expect(result).toEqual(mockAdvertisement.createdAt);
		});
		...
		it("should return createdAt when user is both system and organization administrator", async () => {
			mocks.drizzleClient.query.usersTable.findFirst.mockResolvedValue({
				id: "user123",
				role: "administrator",
				organizationMembershipsWhereMember: [{ role: "administrator" }],
	...
	describe("Where clause coverage", () => {
		it("should execute usersTable where clause with correct parameters", async () => {
			...
			await createdAtResolver(mockAdvertisement, {}, ctx);
