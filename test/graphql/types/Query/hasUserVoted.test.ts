...
suite("Query: hasUserVoted", () => {
    suite("Authentication Tests",()=>{
        ...
        test("returns error with invalid authentication token", async()=>{
            ...
            const hasUserVotedResponse = await mercuriusClient.query(Query_hasUserVoted, {
                            headers: {
    ...
    suite("Authentication Tests",()=>{
        test("returns error if user is not a member of the organization", async()=>{
            ...
            const { postId} = await createPost(adminAuthToken);
            // create a post vote
            const creatPostVoteResult=await mercuriusClient.mutate(Mutation_createPostVote,{
                headers :{
                    authorization : `bearer ${adminAuthToken}`,
                },
                variables:{
...
async function createPost(authToken : string){
    const createOrganizationResult=await mercuriusClient.mutate(Mutation_createOrganization,{
        headers :{
            authorization : `bearer ${authToken}`
        },
        variables:{
