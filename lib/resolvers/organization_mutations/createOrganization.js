...
const createOrganization = async (parent, args, context) => {
  const { User, Organization, Tenant } = context.db;
  ...
  await tenant.save();
  const conn = await addTenantConnection(savedOrg._id);
  let newTenantOrganization = new conn.Organization({
    ...args.data,
    image: uploadImageObj
      ? uploadImageObj.imageAlreadyInDbPath
        ? uploadImageObj.imageAlreadyInDbPath
  ...
  await newTenantOrganization.save();
