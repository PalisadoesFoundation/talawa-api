...
### Issue 1: High Response Time (`totalMs` > 1000ms)
...
### Issue 2: Low Cache Hit Rate (`hitRate` < 0.5)
...
### Issue 3: N+1 Query Pattern (`totalOps` > 50)
...
### Issue 4: Slow Operations in `slow` Array
...
```typescript
app.get('/metrics/perf', async (req, reply) => {
...
2. **Network-Level Restriction (Nginx):**
```nginx
location /metrics/perf {
...
3. **Admin-Only Access:**
```typescript
app.get('/metrics/perf', { preHandler: [requireAdmin] }, async (req, reply) => {
  // ... return metrics
});
```
