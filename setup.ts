...
async function main(): Promise<void> {
  ...
  const { shouldGenerateRefreshToken } = await inquirer.prompt({
    ...
    message: "Would you like to us to auto-generate a new refresh token secret?",
    ...
    if (!isDbEmpty) {
      ...
      if (shouldOverwriteData) {
        const { overwriteDefaultData } = await inquirer.prompt({
          ...
          message: "Do you want to import the required default data to start using Talawa in a production environment?",
          default: false,
        });
