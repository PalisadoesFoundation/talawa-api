...
# Colors for output
# defined in logging.sh (actually logging.sh is plain text, but we remove local defs)
...
if [ "$SKIP_PREREQS" = "true" ]; then
    ...
    if with_spinner "Installing git, jq, curl..." brew install git jq curl; then
...
if [ "$INSTALL_MODE" = "docker" ]; then
    if command_exists docker; then
        ...
        with_spinner "Installing Docker Desktop..." brew install --cask docker
...
if command_exists fnm; then
    ...
    with_spinner "Installing fnm..." brew install fnm
