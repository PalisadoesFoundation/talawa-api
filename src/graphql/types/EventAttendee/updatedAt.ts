...
export const eventAttendeeUpdatedAtResolver = async (
	...
	try {
		...
		ctx.log.error(error);
		throw new TalawaGraphQLError({
			message: "Unexpected error while resolving EventAttendee.updatedAt field",
			extensions: {
				code: "unexpected",
			},
		});
...
EventAttendee.implement({
	fields: (t) => ({
		updatedAt: t.field({
			description: "Date time when the event attendee record was last updated.",
			resolve: eventAttendeeUpdatedAtResolver,
			type: "DateTime",
