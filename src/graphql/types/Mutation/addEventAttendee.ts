...
const mutationAddEventAttendeeArgumentsSchema = z.object({
	data: eventAttendeeInputSchema,
});

/**
 * GraphQL mutation to add an event attendee.
 * Handles both standalone events and recurring event instances.
 */
builder.mutationField("addEventAttendee", (t) =>
	t.field({
		type: User,
		args: {
			data: t.arg({
		...
		description: "Mutation field to add an event attendee.",
		resolve: async (_parent, args, ctx) => {
			...
			const {
				...
				error,
				success,
			} = mutationAddEventAttendeeArgumentsSchema.safeParse(args);
			...
			const userToAdd = await ctx.drizzleClient.query.usersTable.findFirst({
			...
			await ctx.drizzleClient.insert(eventAttendeesTable).values({
				userId: parsedArgs.data.userId,
				eventId: parsedArgs.data.eventId || null,
				recurringEventInstanceId:
					parsedArgs.data.recurringEventInstanceId || null,
				isInvited: false,
				isRegistered: true, // addEventAttendee directly registers the user
				isCheckedIn: false,
				isCheckedOut: false,
			});
