...
type EventAttendanceType = typeof eventAttendancesTable.$inferSelect;

export const registerForEventResolver = async (
	_parent: unknown,
	...
	return await ctx.drizzleClient.transaction(async (tx) => {
		...
		const event = eventRaw as EventType | undefined;
		if (!event) {
			throw new TalawaGraphQLError({
				extensions: {
		...
		const [attendance] = await tx
			...
			.returning();
		return (attendance as EventAttendanceType) ?? null;
	});
};
builder.mutationField("registerForEvent", (t) =>
	t.field({
		...
		resolve: registerForEventResolver,
	}),
);
