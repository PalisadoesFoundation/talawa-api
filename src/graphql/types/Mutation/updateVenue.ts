		...
		resolve: async (_parent, args, ctx) => {
			...
			return await ctx.drizzleClient.transaction(async (tx) => {
				...
				if (parsedArgs.input.attachments?.length) {
					...
					// Verify all files exist in MinIO in parallel before proceeding
					await Promise.all(
					...
					// Delete existing attachments from MinIO and database
					const existingAttachments = existingVenue.attachmentsWhereVenue;
					if (existingAttachments.length > 0) {
						// Remove old files from MinIO (non-blocking, log errors)
						await Promise.all(
							existingAttachments.map(async (oldAttachment) => {
								try {
									...
									ctx.log.warn(
										`Failed to remove old attachment ${oldAttachment.name}: ${error instanceof Error ? error.message : String(error)}`,
									);
								}
							}),
						);
