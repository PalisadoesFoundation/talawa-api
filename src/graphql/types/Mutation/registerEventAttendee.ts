 ...
 */
builder.mutationField("registerEventAttendee", (t) =>
	t.field({
		type: EventAttendee,
		args: {
			data: t.arg({
		...
		description: "Mutation field to register a user for an event.",
		resolve: async (_parent, args, ctx) => {
			...
			const {
				...
				error,
				success,
			} = mutationRegisterEventAttendeeArgumentsSchema.safeParse(args);
			...
			// Check if user to be registered exists
			const userToRegister = await ctx.drizzleClient.query.usersTable.findFirst(
				{
			...
			if (existingAttendee) {
				// If already invited but not registered, update to registered
			...
			// Create new attendee record with registration
