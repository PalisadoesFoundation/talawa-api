...
builder.mutationField("addOrganizationCustomField", (t) =>
    t.field({
        ...
        resolve: async (_parent, args, ctx) => {
            ...
            const organization = await ctx.drizzleClient.query.organizationsTable.findFirst({
                columns: {
                    id: true,
                },
            ...
            const existingField = await ctx.drizzleClient.query.customFieldsTable.findFirst({
                columns: {
                    id: true,
                },
            ...
            if (existingField) {
                throw new TalawaGraphQLError({
                    ...
                    extensions: {
                        ...
                        issues: [{
                            argumentPath: ["name"],
                            message: "A custom field with this name already exists in the organization"
                        }]
                    },
                });
            }
