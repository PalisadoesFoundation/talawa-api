...
export class NotificationEngine {
  ...
  async createNotification(
    ...
    if (!template) {
      throw new Error(`No notification template found for event type "${eventType}" and channel "${channelType}"`);
    }
  ...
  private async createAudienceEntries(
    ...
    if (targetType === NotificationTargetType.ORGANIZATION_ADMIN) {
      ...
      const adminMembers = await this.ctx.drizzleClient.query.organizationMembershipsTable.findMany({
      ...
      const members = await this.ctx.drizzleClient.query.organizationMembershipsTable.findMany({
        columns: { memberId: true },
