...
export const recurringEventExceptionsTableInsertSchema = createInsertSchema(
	eventExceptionsTable,
	{
		exceptionData: (schema) => schema, // JSON object with any structure
		recurringEventInstanceId: (schema) => schema,
		organizationId: (schema) => schema,
		creatorId: (schema) => schema,
		updaterId: (schema) => schema.optional(),
	},
);
