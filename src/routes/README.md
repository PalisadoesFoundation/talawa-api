...
For REST validation, use the helper in `src/routes/validation/zodReply.ts` (`zReplyParsed`). Auth request body schemas (signup, signin, refresh) live in `src/routes/auth/validators.ts`.
