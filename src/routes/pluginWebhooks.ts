 ...
 */
export const pluginWebhooks = fastifyPlugin(async (fastify: FastifyInstance) => {
  // Dynamic webhook endpoint that handles any plugin webhook
  fastify.all("/api/plugins/:pluginId/webhook/*", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { pluginId } = request.params as { pluginId: string };
      const url = request.url;
      
      // Extract the webhook path after /api/plugins/:pluginId/webhook/
      const webhookPath = url.replace(`/api/plugins/${pluginId}/webhook`, '') || '/';
      // Normalize path - ensure it starts with / for root path, otherwise remove leading slash
      const normalizedPath = webhookPath === '/' ? '/' : (webhookPath.startsWith('/') ? webhookPath.slice(1) : webhookPath);
      ...
      if (!webhookHandler) {
        ...
        fastify.log.info(`Available webhook handlers: ${JSON.stringify(Object.keys(extensionRegistry.webhooks?.handlers || {}))}`);
        ...
        reply.status(404).send({ 
          ...
          debug: {
            webhookKey,
            availableHandlers: Object.keys(extensionRegistry.webhooks?.handlers || {}),
            extensionRegistry: {
              hasWebhooks: !!extensionRegistry.webhooks,
              handlersCount: Object.keys(extensionRegistry.webhooks?.handlers || {}).length
            }
          }
        });
        return;
  ...
  fastify.get("/api/plugins/:pluginId/webhook/health", async (request: FastifyRequest, reply: FastifyReply) => {
    const { pluginId } = request.params as { pluginId: string };
