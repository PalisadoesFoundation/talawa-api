...
export async function initializeGenerationWindow(
	...
	try {
		...
		logger.info(
			`Generation window initialized for organization ${input.organizationId}: hotWindowMonthsAhead=${insertedConfig.hotWindowMonthsAhead}, historyRetentionMonths=${insertedConfig.historyRetentionMonths}, currentWindowEndDate=${insertedConfig.currentWindowEndDate.toISOString()}, retentionStartDate=${insertedConfig.retentionStartDate.toISOString()}`,
		);

		return insertedConfig;
	} catch (error) {
		logger.error(
			`Failed to initialize Generation window for organization ${input.organizationId}: ${error instanceof Error ? error.message : String(error)}`,
		);
		throw error;
	}
}
...
export async function extendGenerationWindow(
	...
	try {
		...
		logger.info(
			`Extended Generation window for organization ${organizationId} by ${additionalMonths} months (previousEndDate=${windowConfig.currentWindowEndDate.toISOString()}, newEndDate=${newEndDate.toISOString()})`,
		);

		return newEndDate;
	} catch (error) {
		logger.error(
			`Failed to extend Generation window for organization ${organizationId}: ${error instanceof Error ? error.message : String(error)}`,
		);
		throw error;
	}
}
...
export async function cleanupOldGeneratedInstances(
	...
	try {
		...
		logger.info(
			`Cleaned up ${deletedCount} old Generated instances for organization ${organizationId} (retentionStartDate=${windowConfig.retentionStartDate.toISOString()}, deletedCount=${deletedCount})`,
		);

		return deletedCount;
	} catch (error) {
		logger.error(
			`Failed to cleanup old Generated instances for organization ${organizationId}: ${error}`,
		);
		throw error;
	}
}
