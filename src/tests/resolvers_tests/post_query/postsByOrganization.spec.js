...
beforeAll(async () => {
  ...
  try {
    ...
    await getAccessToken(mainEmail);
    mainUserId = await getUserIdFromLogin(mainEmail);

    const secondEmail = `${shortid.generate().toLowerCase()}@test.com`;
    await getAccessToken(secondEmail);
    secondUserId = await getUserIdFromLogin(secondEmail);

    const thirdEmail = `${shortid.generate().toLowerCase()}@test.com`;
    await getAccessToken(thirdEmail);
    thirdUserId = await getUserIdFromLogin(thirdEmail);
