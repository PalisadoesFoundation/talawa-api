...
const eventProjectMutationHelperFunctions = require('./eventProjectMutationHelperFunctions');
...
beforeAll(async () => {
  ...
  user = await eventProjectMutationHelperFunctions.createUser();
  user2 = await eventProjectMutationHelperFunctions.createUser();
  if (user) {
    org = await eventProjectMutationHelperFunctions.createOrganization(
      user._id
    );
    if (org) {
      event = await eventProjectMutationHelperFunctions.createEvent(
        user._id,
        org._id
      );
      if (event) {
        eventProject =
          await eventProjectMutationHelperFunctions.createEventProject(
            user._id,
            event._id
          );
      }
